...
import { buildPdf, exportFileName } from '@/lib/document-export'
...
export async function GET(_request: Request, { params }: { params: { id: string } }) {
  ...
  const pdf = await buildPdf(doc, { flowchartImage: await loadFlowchartImage(doc) })
  return new NextResponse(pdf, {
    headers: {
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename*=UTF-8''${encodeURIComponent(exportFileName(doc, 'pdf'))}`,
    },
  })
}
